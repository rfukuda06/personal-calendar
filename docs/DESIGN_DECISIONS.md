...
## 13. Email reminders via Render Cron + Gmail SMTP (no queue)

**Chose:** A single `Reminder` table holds stackable per-event reminders (one row per offset). A Render Cron Job runs `npm run reminders:cron` every 5 minutes, queries reminders whose computed fire time falls in a window (`WINDOW_MS`, kept ≥ the cron period so nothing slips between ticks — see `src/lib/reminder-window.ts`), and sends emails through Gmail SMTP (Nodemailer). Idempotency is enforced by a unique `(reminderId, occurrenceKey)` row in `ReminderSend` — for non-recurring events `occurrenceKey` is the parent's ISO start; for recurring events it's the specific instance's ISO start. RRULEs are expanded on-the-fly inside the cron, never materialized as reminder rows. Todos get a single 12pm-LA digest tracked by `(userId, digestDate)` in `TodoDigestSend`.
...
**Why:** For a personal-scale calendar (tens to hundreds of events), polling on a short interval is overwhelmingly the simplest design that meets the requirement. Computing recurring fire times on-the-fly means the cron always reflects the latest RRULE without any "go back and rewrite the materialized rows when someone edits the rule" code. The unique-index approach to idempotency works regardless of whether the parent is recurring or not, with one shape.

**Revised (May 2026) to cut Neon compute cost.** Originally the cron ran every minute with a 90-second window. On Neon's free tier a query every minute never lets the database scale to zero, so its compute stayed awake ~24/7 and exhausted the monthly compute quota — which suspended the DB and made the app look like it had lost all data (storage was fine; only compute was throttled). The cron now runs **every 5 minutes** with the window widened to match (`WINDOW_MS = 6 min`, kept ≥ the cron period so reminders can't fall through the gap; the `ReminderSend` unique key dedupes the wider overlap). The dispatcher also **exits before opening a DB connection between midnight and 6am LA** (`isQuietHour`, checked before any Prisma call — Prisma connects lazily), so Neon stays suspended overnight. Trade-offs: a reminder is delivered within ~5 minutes of its scheduled minute rather than ~1, and anything due during the overnight quiet window is dropped, not replayed at 6am. The timing predicates (`inWindow`, `isQuietHour`, `isDigestWindow`) are pure, unit-tested functions in `src/lib/reminder-window.ts` so the date/DST math stays out of the I/O shell; the noon todo digest gate moved from an exact `minute === 0` check to the same window approach so a delayed tick under the coarser schedule can't skip a day.

**Revised (July 2026) — dropped Resend for Gmail SMTP.** Resend needs a *verified sending domain*, which is really a chain of things that all have to stay green: the domain registration, its WHOIS contact verification, its DNS records, and Resend's own domain check. In June 2026 the registrar (Namecheap) put `personalcal.com` on a WHOIS-verification hold, which swapped its nameservers and wiped every DNS record — so Resend's domain flipped to `failed` and every send was rejected. Worse, the Resend SDK *resolves* (doesn't throw) on a rejected send, and `recordSendOrSkip` only deletes its placeholder row when `send()` throws — so ~7 weeks of undelivered reminders were written to `ReminderSend` as successfully sent, with no error surfaced anywhere. We now send from a real Gmail account over SMTP via **Nodemailer** with an App Password (`GMAIL_USER` / `GMAIL_APP_PASSWORD`): no custom domain means nothing to fall out of verification, and Gmail owns SPF/DKIM. Nodemailer rejects on SMTP failure so `recordSendOrSkip` deletes its row and retries, and `ensureSent` (`src/lib/email-result.ts`, unit-tested) additionally throws when a send reached zero recipients — so the swallowed-failure mode can't recur. Trade-off: Gmail's ~500-recipients/day cap, far above a personal calendar's volume.
